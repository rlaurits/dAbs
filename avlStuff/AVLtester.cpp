...
bool fillMap(AVLMap<double,Data> &bst,map<double,Data> &data) {
...
bool checkEquality(AVLMap<double,Data> bst,map<double,Data> &data) {
    if(bst.size() != data.size()) return false;
    for(auto &p:data) {
        if(data[p.first]!=p.second) return false;
    }
...
bool checkRemove(AVLMap<int,string> bst,vector<int> &v,AVLMap<int,string> &orig,int remKey) {
    for(auto k:v) {
        if(bst[k]!=orig[k]) {
    ...
    if(bst!=orig || !(bst==orig)) {
        cout << "Failed ==/!= check of copy.\n";
    }
    bst.erase(remKey);
    ...
    if(bst==orig || !(bst!=orig)) {
        cout << "Failed ==/!= check of copy after remove.\n";
    }
    return true;
}
...
int helper() {
    ...
    for(auto &p:um2) {
        if(bst2[p.first] != p.second) {
            cout << "Failed recovering after in-order." << endl;
            return 2;
        }
    }
    cout << "Passed balance test 1" << endl;

    bst2.clear();
    for(auto iter=um2.rbegin(); iter!=um2.rend(); ++iter) {
        bst2[iter->first] = iter->second;
    }
    for(auto &p:um2) {
        if(bst2[p.first] != p.second) {
            cout << "Failed recovering after reverse order." << endl;
            return 2;
        }
    }
    cout << "Passed balance test 2" << endl;

    bst2.clear();
    string val = "A";
    for(int i=0; i<1000000; ++i) {
        bst2.insert(make_pair(i,val));
        bst2.insert(make_pair(2000000-i,val));
    }
